...
export class SettingsService {
  ...
  async getSettingsStats(userId: string): Promise<StatsResponse> {
    ...
    const documents = await this.prisma.document.findMany({
      where: { 
        userId, // Check directly on userId
    ...
    for (let i = 4; i>=0; i--) {
      ...
      const yearTotal = filings
        ...
        .reduce((sum, f) => sum + (f.amount || 0), 0);
      yearlyData.push({ year: year.toString(), amount: Math.abs(yearTotal) });
    }
    ...
    filings
      ...
      .forEach(f => {
        ...
        if (ded.donationAmount) deductionBreakdown['Donations'] = (deductionBreakdown['Donations'] || 0) + Number(ded.donationAmount);
        if (ded.medicalExpenses) deductionBreakdown['Medical'] = (deductionBreakdown['Medical'] || 0) + Number(ded.medicalExpenses);
      });

    const pieData: DeductionData[] = Object.keys(deductionBreakdown).map(name => ({ name, value: deductionBreakdown[name] }));

    return {
      cards: {
        totalRefunds: currentYearRefunds, refundGrowth, filedReturns: totalFiled,
        filingSince: firstFilingDate ? firstFilingDate.getFullYear() : 'N/A',
        avgProcessingDays, daysFaster, docsUploaded,
      },
  ...
  async getUserDocuments(userId: string) {
    // We can now query directly by userId
    return this.prisma.document.findMany({
      where: { userId },
      orderBy: { createdAt: 'desc' },
      include: { filing: { select: { id: true, filingId: true } } }
    });
  }
  ...
  async uploadDocument(userId: string, file: Express.Multer.File, filingId: string | null, name?: string) {
    ...
    // If filingId is provided, verify ownership
    if (filingId) {
    ...
    // FIX: Added userId to the create call
    return this.prisma.document.create({
      data: { 
        name: docName, 
        url: fileUrl, 
        type: file.mimetype, 
        filingId, // Can be null
  ...
  async downloadDocument(userId: string, docId: string): Promise<StreamableFile> {
    ...
    // FIX: Check ownership via userId directly
  ...
  async deleteDocument(userId: string, docId: string) {
    ...
    // FIX: Check ownership via userId directly
    if (!doc || doc.userId !== userId) throw new ForbiddenException('Access denied');
